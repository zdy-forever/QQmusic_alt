...
- 搜索栏旁边可以在 QQ 音乐、网易云音乐和 Apple Music 之间切换。网易云音乐当前支持搜索、歌词、播放链接、手机号验证码登录、网页登录 Cookie 导入、歌单读取、歌单详情，以及自建歌单的新建、重命名、删除、加入歌曲和移除歌曲。Apple Music 当前使用公开试听模式，支持搜索和在线播放试听片段，不支持下载、歌词、完整播放或用户歌单。
...
在搜索栏旁边可以切换音乐平台，切换后搜索、歌词、播放、登录和歌单都会走对应平台。Apple Music 使用公开试听接口，无需登录。点击“设置”可以调整歌曲队列显示内容，例如是否显示歌手、专辑名、歌曲时长、MID，以及队列字体大小。设置会保存到 `.qqmusic_settings.json`，默认音质、下载目录和启动时是否自动同步歌单也会一起保存。
